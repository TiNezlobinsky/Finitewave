...
def _local_velocity(act_t):
    ...
    for i in range(1, N1 - 1):
        for j in range(1, N2 - 1):
            times = [act_t[i - 1, j], act_t[i + 1, j], act_t[i, j - 1], act_t[i, j + 1]]
            if times:
                min_t = np.min(times)
...
class Velocity2DTracker(ActivationTime2DTracker):
    ...
    ----------
    file_name : str
        Name of the file where the velocity data will be saved.
        Default is "front_velocity".
    """
    ...
    def compute_velocity_front(self):
        ...
        ind_stim = np.argwhere(act_t == np.min(act_t[act_t >= 0.0]))

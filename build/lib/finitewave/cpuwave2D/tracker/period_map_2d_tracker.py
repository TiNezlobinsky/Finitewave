...
class PeriodMap2DTracker(Animation2DTracker):
    ...
    def track(self):
        if self._t > self.step:
            active_nodes = np.logical_and(
                self._period_map_state == 1, self.model.u > self.threshold)
            self.period_map[active_nodes] = self.model.t - \
                self._last_time_map[active_nodes]
            self._last_time_map[active_nodes] = self.model.t
            self._period_map_state[active_nodes] = 0
            self._period_map_state[np.logical_and(
                self._period_map_state == 0, self.model.u < self.threshold)] = 1

            np.save(os.path.join(self.path, self.dir_name,
                    str(self._frame_n)), self.period_map)
            self._frame_n += 1
            self._t = 0
        else:
            self._t += self._dt
